...
export async function sendShipWeekLaunchEmail(email: string, name: string) {
  if (!resend) return;
  try {
    await resend.emails.send({
      from: "Codeown Competition <competition@codeown.space>",
      to: email,
      subject: "The Ship Week is LIVE! 🚀",
      html: `
        <div style="font-family: sans-serif; padding: 40px; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 20px;">
          <div style="background: rgba(255, 215, 0, 0.1); color: #FFD700; padding: 4px 12px; border-radius: 100px; display: inline-block; font-size: 12px; font-weight: 800; margin-bottom: 24px;">WEEKLY MISSION</div>
          <h1 style="color: #0f172a; margin: 0 0 16px 0; font-size: 32px; letter-spacing: -0.04em;">Ready to ship, ${name}?</h1>
          <p style="font-size: 16px; color: #475569; line-height: 1.6; margin-bottom: 32px;">
            A new <strong>Ship Week</strong> has officially started. You have 7 days to build and showcase one major feature.
          </p>
          <div style="background: #f8fafc; padding: 24px; border-radius: 12px; margin-bottom: 32px;">
            <p style="margin: 0; font-size: 14px; font-weight: 700; color: #0f172a;">GOAL: Share 3 updates (type: Update or Milestone) before Sunday to qualify.</p>
          </div>
          <div style="text-align: center;">
            <a href="https://codeown.space/ship-week" style="padding: 14px 40px; background: #000000; color: white; text-decoration: none; border-radius: 12px; font-weight: 700; display: inline-block;">View Week Dashboard &rarr;</a>
          </div>
        </div>
      `
    });
  } catch (err) {
    console.error("Error sending ship week launch email:", err);
  }
}

export async function sendShipWeekReminderEmail(email: string, name: string, currentCount: number) {
  if (!resend) return;
  try {
    await resend.emails.send({
      from: "Codeown Competition <competition@codeown.space>",
      to: email,
      subject: "3 Days Left! Are you shipping? 🔥",
      html: `
        <div style="font-family: sans-serif; padding: 40px; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 20px;">
          <h2 style="color: #0f172a; margin: 0 0 16px 0;">Don't miss the deadline, ${name}!</h2>
          <p style="font-size: 16px; color: #475569; line-height: 1.6;">
            You've shared <strong>${currentCount}/3</strong> ships so far. You need ${3 - currentCount} more to qualify for the weekly competition.
          </p>
          <div style="text-align: center; margin-top: 32px;">
            <a href="https://codeown.space/ship-week" style="padding: 14px 40px; background: #000000; color: white; text-decoration: none; border-radius: 12px; font-weight: 700; display: inline-block;">Qualify Now &rarr;</a>
          </div>
        </div>
      `
    });
  } catch (err) {
    console.error("Error sending ship week reminder email:", err);
  }
}

export async function sendShipWeekWinnerEmail(email: string, name: string, winnerName: string, projectTitle: string) {
  if (!resend) return;
  try {
    await resend.emails.send({
      from: "Codeown Competition <competition@codeown.space>",
      to: email,
      subject: "The Ship Week Winner Revealed! 👑",
      html: `
        <div style="font-family: sans-serif; padding: 40px; max-width: 600px; margin: 0 auto; border: 1px solid #FFD700; border-radius: 24px; background: linear-gradient(to bottom right, #ffffff, #fffdf0);">
          <div style="text-align: center; margin-bottom: 32px;">
             <span style="font-size: 48px;">👑</span>
          </div>
          <h1 style="color: #0f172a; text-align: center; font-size: 28px; margin-bottom: 16px;">We have a winner!</h1>
          <p style="font-size: 18px; color: #475569; line-height: 1.6; text-align: center; margin-bottom: 32px;">
            Congratulations to <strong>${winnerName}</strong> for winning the week with <strong>"${projectTitle}"</strong>!
          </p>
          <div style="text-align: center;">
            <a href="https://codeown.space/ship-week" style="padding: 14px 40px; background: #000000; color: white; text-decoration: none; border-radius: 12px; font-weight: 700; display: inline-block;">View Hall of Fame</a>
          </div>
        </div>
      `
    });
  } catch (err) {
    console.error("Error sending ship week winner email:", err);
  }
}
